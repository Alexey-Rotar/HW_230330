...
def CutSymbols(string, symbols):
    string = list(string.upper())
    for item in string.copy():
        if item in symbols:
            string.remove(item)
    string = ''.join(string)
    return string
...
only_vowels = CutSymbols(poem, '-БВГДЖЗЙКЛМНПРСТФХЦЧШЩ')
only_vowels = only_vowels.split()

is_rhythm = True;
for item in only_vowels:
    if len(only_vowels[0]) != len(item):
        is_rhythm = False;
